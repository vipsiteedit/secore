...
if ($id) {
    ...
    $posts->find($id);
    $showshort = ($posts->showshort) ? 'yes' : 'no';
...
if (isRequest('GoTonewblog') && ($user_id || ($user_group == 3))) {
    ...
    if (empty($anons)) {
        $errortext .= "\r\n" . $section->language->lang062;
    }
    if (empty($full)) {
        $errortext .= "\r\n" . $section->language->lang063;
    }
//    if (($errortext == "") && 0) {
    if ($errortext == "") {
        $posts = new seTable('se_blog_posts');
...
/*/
        ...
        $posts->full = $full;
//*/
       // $posts->show_short = ($posts->show_short != 'N');

        $posts->tags = $tegi;
        $posts->commenting = $razcom;
        $posts->comment_presence = $skrit;
        $posts->addField('showshort','TINYINT(1) NOT NULL default 1 AFTER  `event`');
        $posts->showshort = ($showshort == 'yes') ? 1 : 0;
        $posts->keywords_post = $keywords;
        $posts->description_post = $description;
        $posts->event = ($event) ? 'Y' : 'N';
...
//*/
    ...
    list($ehour, $eminute, $eday, $emonth, $eyear) = explode('.', date("H.i.d.m.Y"));
}
//if (!$event) {
