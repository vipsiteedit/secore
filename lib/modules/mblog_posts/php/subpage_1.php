...
$date_add = $posts->date_add;
$blog_date = date('d', strtotime($posts->date_add));
...
$tagcount = 0;
list($date_add_day, $date_add_nedel, $date_add_god) = bl_getFormatDate($section, $date_add);
...
if (isRequest('GoTonewbbs')) {
    ...
    if (isset($pin)) {
        if (empty($pin)) {
            $errstpin = "errorinp";
            $errorpin = $section->parametrs->param9;
            $err = true;
        } else {
    ...
    if (!$err) {
        ...
        if (seUserName() == "") {
            ...
            if (empty($name)) {
                $nameauthor = $section->parametrs->param12;
            } else {
                $nameauthor = $name;
            }
        } else {
