...
function addAjaxCart(data){
    var new_add = '<div class="goodInfo updAjax" data-id="' + data.id + '"> ' +
        ...
        ' <a class="linkDelGood" href="javascript:void(0);" title="' + flycart.param11 + '">&nbsp;</a> ' +
        ' </div>';
...
$('body').on('submit', '.form_addCart', function(e){  
    console.log('form_addCart submit');
    e.preventDefault();
    if (ajax_request == true){
        var product = $(this).closest('.blockGoods, .productItem, .goodsDetail, .theGoodContent, .blockAllItem:not(.emptyGoods), .specialItem, .goodsContent').first(),
        ...
        $.ajax({
            url: ajax_url,
            data: product.find('.form_addCart').serialize(),
            type: 'post',
            dataType: 'json',
            success:function(result){
                if (!result) return;
                var clone = object.clone(),
                    x = object.offset(), 
                    x2 = cart.last().offset();
                if (result.incart && (flycart.param22 == 'f' || flycart.param22 == 'fm')){
                ...
                if (product.closest('.notAjaxCart').length)
                    document.location.href='/'+flycart.param4+'/';
                if (result.product && (flycart.param22 == 'm' || flycart.param22 == 'fm')){
