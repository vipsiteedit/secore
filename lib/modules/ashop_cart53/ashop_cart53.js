var ashop_cart53_execute = function(params){ 
        ...
        for (var i = 0; i < list.length; i++){
            if (i in list) {
                ...
                if (item.length > 0){
                    ...
                    if (item.find('.itemOldPrice').length == 0)
						item.find('.itemNewPrice').before('<span class="itemOldPrice"></span> ');
					item.find('.itemOldPrice').show().html(list[i].oldprice);
					item.find('.itemNewPrice').html(list[i].newprice);
					if (list[i].newprice == list[i].oldprice)
						item.find('.itemOldPrice').hide();
                }
                else{
...
$(document).ready(function () {
    ...
    $('#blockCartDelivery .blockCartList').on('change', '.radioDeliveryType', function(){             
        ...
        $(this).closest('.deliveryType').find('.subdeliveries').show();
    ...
    $('#blockCartDelivery').on('change', '.subdeliveries input', function(){             
        $.ajax({
            data: $(this).serialize(),
            overlay: $('#blockCartDelivery .blockCartList')
        });
