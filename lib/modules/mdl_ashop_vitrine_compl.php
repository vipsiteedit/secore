<?php
function module_ashop_vitrine_compl($razdel, $section = null)
{
 ...
 if (strpos(dirname(__FILE__),'/lib/modules'))
   $__MDL_URL = 'lib/modules/ashop_vitrine_compl';
 else $__MDL_URL = 'modules/ashop_vitrine_compl';
 $__MDL_ROOT = dirname(__FILE__).'/ashop_vitrine_compl';
 ...
 if ((isRequest('ajax'.$razdel)) && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
     ...
     elseif (isRequest('preorder') && $section->parametrs->param336 == 'Y'){
         if ($section->parametrs->param346 == 'Y' || $section->parametrs->param348 == 'Y') {
             if ($section->parametrs->param346 == 'Y' && !isRequest('personal_accepted')) {
                 $response['error'] = $section->language->lang121;    
             }
         
             if ($section->parametrs->param348 == 'Y' && !isRequest('additional_accepted')) {
                 $response['error'] = $section->language->lang121;    
             }
         }
 ...
 //BeginSubPageoption
 $__module_subpage['option']['admin'] = "";
 $__module_subpage['option']['group'] = 0;
 $__module_subpage['option']['form'] =  '';
 if($razdel == $__data->req->razdel && !empty($__data->req->sub)
 && $__data->req->sub=='option' && file_exists($__MDL_ROOT . "/tpl/subpage_option.tpl")){
	include $__MDL_ROOT . "/php/subpage_option.php";
	ob_start();
	include $__data->include_tpl($section, "subpage_option");
	$__module_subpage['option']['form'] =  ob_get_contents();
	ob_end_clean();
 } //EndSubPageoption
 //BeginSubPagespecial
