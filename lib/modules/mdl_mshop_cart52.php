<?php
function module_mshop_cart52($razdel, $section = null)
{
 ...
 if (strpos(dirname(__FILE__),'/lib/modules'))
   $__MDL_URL = 'lib/modules/mshop_cart52';
 else $__MDL_URL = 'modules/mshop_cart52';
 $__MDL_ROOT = dirname(__FILE__).'/mshop_cart52';
 ...
 $plugin_cart = new plugin_shopcart53($options_cart);
 ...
 $plugin_delivery = new plugin_shopdelivery53($not_delivery);
 ...
 if ($ajax){ 
     ...
     if ($total['count'] > 0 && (isRequest('delcartname') || isRequest('countitem') || isRequest('set_region') || isRequest('getcart'))){
         ...
         foreach($plugin_delivery->getDeliveryList($not_delivery) as $key => $val){
             ...
             $html .= "<div class=\"deliveryTypePriceTime\"><span class=\"deliveryTypePrice\">{$val['price']}</price>, <span class=\"deliveryTypeTime\">".getTimeWord($section, $val['time'])."</span></div>\n";
