...
if ($section->parametrs->param80 != 'Y') { // обычные аналоги 
    ...
    if ((isset($id_analog)) && (!empty($id_analog))) {
        $shop_price->select('sp.*, sg.name as `group`');
        $shop_price->Where("sp.lang = '?'", $lang);
        $shop_price->andWhere("sp.enabled = 'Y'");
        $shop_price->leftjoin('shop_group sg', 'sg.id = sp.id_group');
        $shop_price->andWhere("sp.id_analog = '?'", $id_analog);
        $shop_price->andWhere("sp.id  <> ?", $viewgoods);
