...
$thisgroup_name = $shop_variables->parseGroupText($group['name'], $group);
    
if (trim($group['title']) == '') {
    $group['title'] = $thisgroup_name;
}

$thisgroup_name = $shop_variables->parseGroupText($group['page_title'], $group) . $add_title;
...
$related = $psg->getRelated((int)$group['id']);
