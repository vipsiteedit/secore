...
function getRegionName($full = false){
    ...
    $tlb_region->select(" c.name as country, r.name as region, (SELECT t.name FROM town AS t WHERE t.id_region=r.id AND t.id=$city_id LIMIT 1) as city");
    $tlb_region->innerJoin('country c', 'c.id=r.id_country');
    $tlb_region->where("r.id = ?", $region_id);
    $tlb_region->andwhere("c.id = ?", $country_id);
    $tlb_region->fetchOne();
    if ($tlb_region->isFind()){
        if (!$full) {
            ...
            $region_city = $tlb_region->country . ', ' . $tlb_region->region . ', ' . $tlb_region->city; 
        }      
    }
    return $region_city;
}
...
if (!function_exists('saveUserFields')){
function saveUserFields($id_order) {
