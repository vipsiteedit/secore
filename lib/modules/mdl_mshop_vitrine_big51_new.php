<?php
function module_mshop_vitrine_big51_new($razdel, $section = null)
{
 ...
 if (strpos(dirname(__FILE__),'/lib/modules'))
   $__MDL_URL = 'lib/modules/mshop_vitrine_big51_new';
 else $__MDL_URL = 'modules/mshop_vitrine_big51_new';
 $__MDL_ROOT = dirname(__FILE__).'/mshop_vitrine_big51_new';
 ...
 if ((isRequest('ajax'.$razdel)) && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
     ...
     elseif (isRequest('compare') && $section->parametrs->param269 == 'Y'){
         ...
         $response['compare'] = $plugin_compare->changeCompare($id_price);
     ...
     elseif (isRequest('quickview') && $section->parametrs->param327 == 'Y'){
         ...
         include $__MDL_ROOT . '/tpl/subpage_show.tpl';
 ...
 if (isRequest('shopcatgr')) {
     ...
     if ($shopcatgr) {
         ...
         $tbl->find($shopcatgr);
         $catgr = $tbl->code_gr;
         $__data->go301(seMultiDir().'/'.$_page.'/cat/'.urlencode($catgr).'/');
     } 
     else {
 ...
 if (isRequest('GoToComment') && ($user_group || $section->parametrs->param217 == 'C')) {
     ...
     if (!$user_group) {
         ...
         if (!$comment_user)
             $error_comm_message = 'Необходимо указать ваше имя.'; 
         
         require_once getcwd() . '/lib/card.php';
         if (!checkcard(getRequest('comm_captcha', 1)))
             $error_comm_message = 'Неверно введен код с картинки.';   
     }
