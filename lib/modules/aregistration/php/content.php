...
if (isRequest('GoToAuthor')) {
    ...
    if ($section->parametrs->param51 == 'Y' || $section->parametrs->param53 == 'Y') {
        if ($section->parametrs->param51 == 'Y' && !isRequest('personal_accepted')) {
            $errorlicense = $section->language->lang026; 
            $err = true;   
        }
        
        if ($section->parametrs->param53 == 'Y' && !isRequest('additional_accepted')) {
            $errorlicense = $section->language->lang026; 
            $err = true;   
        }
    }
...
//                    $SESSION_VARS['ADMINUSER']  = $section->parametrs->param3;
                ...
                if (!empty($section->parametrs->param5))
                    $url = seMultiDir() . '/' . delExtFile($section->parametrs->param5) . URL_END;
                else
                    $url = seMultiDir() . '/'  .$_page . '/' . $razdel . '/sub1/';
