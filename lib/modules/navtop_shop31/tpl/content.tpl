...
<div id="bs-menu-collapse" class="row collapse">
    <nav class="headerCatalog">
       <? if(function_exists('getItemMenu')){ list($menuitems) = getItemMenu($section->parametrs->param16,$section->parametrs->param18,$section->parametrs->param19); $__data->setList($section,'menuitems', $menuitems);} ?>
       <ul class="groupList">
       <?php foreach($section->menuitems as $item): ?>
           <li class="headerCatalogItem horiz <?php if($item->name==strval($section->parametrs->param1)): ?>catalog<?php endif; ?>" data-id="<?php echo $item->name ?>">
               ...
               <span class="text <?php if(strval($section->parametrs->param7)=='1' || $item->image==''): ?>noimg<?php endif; ?>">
