...
foreach($pricelist as $goods) {                                                            
    ...
    $goods['price_opt'] =  $plugin_amount->showPriceOpt(true, $rounded, $separator);   
    
    $goods['price_corp'] =  $plugin_amount->showPriceCorp(true, $rounded, $separator);  
    
    $goods['price_step'] = $plugin_amount->showPrice(true, $rounded, $separator, true);
    
    $goods['price_opt_step'] =  $plugin_amount->showPriceOpt(true, $rounded, $separator, true);   
    
    $goods['price_corp_step'] =  $plugin_amount->showPriceCorp(true, $rounded, $separator, true);  
    ...
    $goods['price_label'] = $psg->getPriceLabel($goods['id'], (string)$section->language->lang008);
    
    if ($section->parametrs->param345 == 'Y' && !empty($goods['labels'])) {
        foreach($goods['labels'] as $val) {
            $__data->setItemList($section, 'labels' . $goods['id'], $val);
        }
        $goods['labels'] = true;
    }
    else {
