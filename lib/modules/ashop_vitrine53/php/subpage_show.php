...
$price_opt =  $plugin_amount->showPriceOpt(true, $rounded, $separator);   
    
$price_corp =  $plugin_amount->showPriceCorp(true, $rounded, $separator);  

$price_step = $plugin_amount->showPrice(true, $rounded, $separator, true);
    
$price_opt_step =  $plugin_amount->showPriceOpt(true, $rounded, $separator, true);   
    
$price_corp_step =  $plugin_amount->showPriceCorp(true, $rounded, $separator, true);  
...
$product_group = $price_fields['group_name'];

$product_measure = $price_fields['measure'];
$product_name = $shop_variables->parseProductText($price_fields['name'], $price_fields);
...
if(!empty($extra_photo)) {
    $i = 0;
    foreach ($extra_photo as &$val) {
        if (empty($val['alt'])) {
            $i++;
            $val['alt'] = $product_name . ' - картинка ' .  $i;
        }
        $__data->setItemList($section, 'photos', $val);
    }
