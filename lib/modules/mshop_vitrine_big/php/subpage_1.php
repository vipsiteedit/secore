...
$extra_photo =  $psg->viewgalleryImages($price_fields['id'], $section->parametrs->param285, $section->parametrs->param286, $section->parametrs->param293, 'w', $section->parametrs->param292);// viewGalleryImages($section, $extra_photo);
if($extra_photo!='0')
...
//=====================
    ...
    $list_cart_param = join(',',$_SESSION['SHOP_VITRINE']['selected'][$price_fields_id]);
    $plugin_amount = new plugin_shopAmount($price_fields['id'], '', $ptype, 1,
...
// Путь по каталогу к товару
        ...
        if ($old_format) {
            $path = new plugin_ShopCatPath($title,'/');
