...
class payment_paypal extends basePayment{
  
  public function setVars()
  {
		return array('pp_user'=>'API UserName', 'pp_passw'=>'API Password', 'pp_secret'=>'API Signature');
  }   
  ...
  public function blank($pagename) 
  {
		...
		$user = $macros->execute('[SETCURRENCY:'.$currency.'][PAYMENT.PP_USER]');
		$currency = ($currency == 'RUR') ? 'RUB' : $currency;
		$pass = $macros->execute('[PAYMENT.PP_PASSW]'); 
		$signature = $macros->execute('[PAYMENT.PP_SECRET]');
  ...
  public function success()
  {
		...
		if($_GET['token'] && $_GET['PayerID']){
			$user = $macros->execute('[PAYMENT.PP_USER]');
			$pass = $macros->execute('[PAYMENT.PP_PASSW]'); 
			$signature = $macros->execute('[PAYMENT.PP_SECRET]');
			$paypal_server = ($this->test) ? 'sandbox' : 'live';
			...
			if(is_array($getDetail) && $getDetail['ACK'] == 'Success') {
				...
				if(is_array($response) && $response['ACK'] == 'Success') {
					...
					$this->success = '<h4 class="contentTitle">Платеж принят</h4>
						<p>Ваш заказ № [ORDER.ID] принят</p>
						<table class="tableTable" border="0">
							<tbody class="tableBody">
								<tr><td>Номер транзакции:</td><td>'.$response['PAYMENTINFO_0_TRANSACTIONID'].'</td></tr>
								<tr><td>Дата и время платежа:</td><td>'.date('r',strtotime($response['PAYMENTINFO_0_ORDERTIME'])).'</td></tr>
							</tbody>
						</table>';
				} else {
					$this->success = '<h4 class="contentTitle">Ошибка в проведении платежа</h4>
						<table class="tableTable" border="0">
							<tbody class="tableBody">
								<tr><td>Причина:</td><td>'.$response['L_SHORTMESSAGE0'].'</td></tr>
								<tr><td>Результат:</td><td>'.$response['L_LONGMESSAGE0'].'</td></tr>
							</tbody>
						</table>';
				}
