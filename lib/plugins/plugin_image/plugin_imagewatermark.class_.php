...
class plugin_imagewatermark {
  ...
  public function execute($source, $watermark, $watermarktype='text', $color = 0x0000FF, $alpha_level = 100, $dest, $size, $res= 's', $pos = 'center'){
	...
	$main_img_obj = $this->rasampled($main_img_obj, $ratio);
	if ($ext != 'gif'){
	  if ($watermarktype == 'text') {
	    $font = 'lib/fonts/arial.ttf';
	    $img_obj_new = $this->create_watermark( $main_img_obj, $watermark, $font, 255, 255, 255, $alpha_level);
		...
		if ($watermark) {
		    ...
		    $img_obj_new = $this->joinimage( $main_img_obj, $ext, $watermark_img_obj, $alpha_level, $pos); //$alpha_level
		}
	  }
	} else {
